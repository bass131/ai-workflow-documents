...
사이트 본문은 독자가 필요한 개념과 방법을 찾아 쓰는 위키로 작성한다. 개인 일지나 대화 경과보다 정의·역할·적용 조건·예시·한계를 중심에 두고, 사례의 관찰 날짜와 근거는 보존한다. 구체적인 기준은 작성 가이드의 ‘위키 문체와 문서 역할’을 따른다.
...
표와 개념 설명에서는 단계·핵심 판단과 보충 설명을 구별한다. 굵기·배치·절제된 색상을 함께 쓰되 색 없이도 의미가 전달되어야 한다. 문서를 작성하거나 고친 뒤에는 Computer Use로 실제 화면을 다시 읽고, 기존/변경 또는 문제/제안의 구분과 핵심이 바로 보이는지 확인한다. 구분이 흐리면 고치고 다시 확인하며, 세부 기준은 작성 가이드의 ‘강조와 비교’ 및 ‘화면에서 다시 읽기’를 따른다.
