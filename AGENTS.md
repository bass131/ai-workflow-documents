...
스택은 Astro + Starlight + TypeScript, npm이다. 정확한 버전은 package.json / package-lock.json, 실행 Node 계열은 .node-version에 있다. 홈은 src/pages/index.astro, 문서는 src/content/docs, 메뉴는 astro.config.mjs, 공개 파일은 public, 검증 도구는 scripts에 있다.
